...
#############################################################################
def read_nested_files(filename):
    """
    ...
    try:
        ...
        sys.exit(0)
    status('reading from file %s' % (filename))

    list_of_lines = []
    for line in infile:
        if line.upper().startswith('*INCLUDE'):
            parameters = getparameters([line])
            list_of_lines_nested = read_nested_files(parameters['INPUT'])
...
def read_cline_file(filename):
    ...
    for line in infile:
        ...
        if len(coords) == 3:
            add_node = coords + array([1.0, 0.0, 0.0])
...
def generous_round(t):
    ...
    """
    return int(10**round(log10(t)+0.5))
#############################################################################
...
if args.centerline:
    cline_coords = read_cline_file(args.centerline)
...
list_of_lines = read_nested_files(infilename)
keywords = parse_abaqus_input_file(list_of_lines)
status('%d ABAQUS keywords found.' % (len(keywords)))
...
if args.sections:
    status('deleting all irrelevant items')
    becas_elsets_shell = []
    becas_elsets_3D = []
    ...
    for elset in elset_definitions.keys():
        if re.search(pattern, elset):
            ...
            becas_elsets_3D.append(elset + '_3D')

    becas_elsets_shell.sort()
    becas_elsets_3D.sort()
...
status('computing effective layup')
...
status('equalizing layup: %d layers everywhere' % (args.layers))
equalized_layup = {} 
for elset, layup in layup_of_elset.items():
...
########################################
status('determining layer thicknesses')
for current_elset in elsets_to_offset:

    status('processing Element Set %s' % (current_elset));
...
status('computing the offset vectors')
...
status('generating hexahedral elements')
for current_elset in elsets_to_offset:
...
status('generating new elset and nset definitions')
...
status('correcting mid-side nodes')
for current_elset in elsets_to_offset:
...
if args.sections:
    status('generating 2D mesh for BECAS')
    i = 0
    for elset in becas_elsets_3D:
...
# If in debug mode: write the new Abaqus input file
###################################################
if args.debug:
    outfilename = 'shellexpander_debugfile.inp'
    outfile = open(outfilename, 'w')
    status('debugging mode: writing abaqus input file %s' % (outfilename))
...
if args.sections:
    status('writing BECAS input files')
    ...
    i=0
    for elset_shell, elset_3D in zip(becas_elsets_shell, becas_elsets_3D):
        status('Processing section %s' % (elset_shell))
        becas_elements = new_elset_definitions[elset_3D]
