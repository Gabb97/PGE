function fixed_frames(PathStruct, hub , dat_file_name)
...
% Send status #LS
disp('  Writing Fixed_frames.dat file.........')
...
%--------------------------------------------------------------------------
% I skip this part in this version, as we never use imbalance and failure
% in our PGE/WTD course. This means I set the variable z_temp = 2, as if the USer had chosen
% 'NO' in the options.
% I add nevertheless a warning informing the User. #LS

% z_temp = menu ('IMBALANCE & FAILURE MODELLING ?' , 'YES' , 'NO' );
z_temp = 2; % This forces to exclude imbalance and failure models #LS
...
% I add the warning here 
disp(' ')
disp (' WARNING: Imbalance and failure models are disabled in this version of the Pre-Processor')
disp(' ')
...
if hub.number_of_blades == 2,
    ...
    if z_temp == 1 ; %% MEANS IMBALANCE AND FAILURE
        
        Rc2_temp_1 = rot_o((+180*pi/180+imbalance_failure.azimut_angle_err(2))*[-sin(hub.tilt_from_horizontal_rad) ; cos(hub.tilt_from_horizontal_rad) ; 0 ])*R11_precone ;
        Rc2_temp_2 = Rc2_temp_1 * rot_o(((-pi/2)-hub.blade_set_angle) * [ 1 ; 0 ; 0 ]);
        Rc2 = Rc2_temp_2 * rot_o((imbalance_failure.set_angle_err(2)) * [ 1 ; 0 ; 0 ]);
        
    else
        
        Rc2_temp_1 = rot_o(180*pi/180*[-sin(hub.tilt_from_horizontal_rad) ; cos(hub.tilt_from_horizontal_rad) ; 0 ])*R11_precone ;
        Rc2_temp_2 = Rc2_temp_1 * rot_o(((-pi/2)-hub.blade_set_angle) * [ 1 ; 0 ; 0 ]);
        Rc2 = Rc2_temp_2 ;
        ...
        Rc2_check = rot_o(180*pi/180*[-sin(hub.tilt_from_horizontal_rad) ; cos(hub.tilt_from_horizontal_rad) ; 0 ])*R2 ;
...
elseif hub.number_of_blades == 3,
    ...
    if z_temp == 1 ; %% MEANS IMBALANCE AND FAILURE
        ...
        % 20.june.2010 add azimuthal imbalances
        
        Rc2_temp_1 = rot_o((+120*pi/180+imbalance_failure.azimut_angle_err(2))*[-sin(hub.tilt_from_horizontal_rad) ; cos(hub.tilt_from_horizontal_rad) ; 0 ])*R11_precone ;
        Rc3_temp_1 = rot_o((-120*pi/180+imbalance_failure.azimut_angle_err(3))*[-sin(hub.tilt_from_horizontal_rad) ; cos(hub.tilt_from_horizontal_rad) ; 0 ])*R11_precone ;
        %
