...
%   $Revision: 1.4.4.1 $  $Date: 2009/01/08 18:57:34 $

if nargin<3||isempty(type)
   type = 'tp';
else
   if ~strcmp(type(1:2),'tp')
      error('SPLINES:STMAK:unknowntype',...
           ['Cannot handle the specified type, ''',type,'''.'])
   end
...
if ~exist('der','var')
   ...
   otherwise
      error('SPLINES:STMAK:atmostbivar',...
            'Cannot handle functions of more than 2 variables.')
   end
end

if nco~=nce+exces
   error('SPLINES:STMAK:centersdontmatchcoefs',...
        ['Number of centers and coefficients',...
...
if nargin<4||isempty(interv)
   ...
   try
      ...
      error('SPLINES:STMAK:wronginterv',...
           ['INTERV must be a cell array of size ',...
                     '[1,size(CENTERS,1)],\n with each INTERV{i} ',...
                     'of the form [a,b] with a<=b.'],0)
   end
end
