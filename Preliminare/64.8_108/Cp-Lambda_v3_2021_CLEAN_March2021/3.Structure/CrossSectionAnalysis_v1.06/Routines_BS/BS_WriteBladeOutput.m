...
% ========================================================================
% 
% 04.06.2018 -  Modified to fix some bugs in the writing of
%               'hub_details.txt'
%
% ========================================================================
...
if Input.Files4PPP == 1
    ...
    % ==== Hub details ====================================================
    disp(' ')
    disp('Please identify the input folder of CP-Lambda Pre-processor, or close this window to generate the files in the Output folder.');
    disp(' ')
    
%     [hub_file,ppp_path,~] = uigetfile('.\..\..\1.PreProcessor_v530_c2018\input\*.txt');
    ppp_input_path = uigetdir('.\..\..\1.PreProcessor_v541_c2020');
    ...
    if ppp_input_path == 0
       ...
       output_path = ppp_input_path;
    end
   
    % ==== Hub details ====================================================
    if ppp_input_path ~= 0
        
        % Identify the hub_details.txt file and check existence
        file = [ppp_input_path  '\hub_details.txt'];
        
        if exist(file) == 0
            disp(' ')
            disp('WARNING: Unable to find hub_details.txt in the selected folder');
            disp(' ')
        else
             % open hub_details.txt and update the rotor diameter
            ...
            for il = 1: iline
                if ~isempty(strfind(data{il},'Rotor diameter'))
    ...
    f=fopen(File,'w');
        ...
        fprintf(f,formatH,'% Distance from root [-]','Chord [m]','Twist [deg]','Thickness [%]','Pitch axis [%]','PreBend [m]','Airfoil');
        
        fprintf(f,'\n');
        fprintf(f,'\n');
        
        formatD='%-10.4f %-15.6f %-15.6f %-15.6e %-15.6f  %-15.6f %d \n' ;
        
        for i=1:length(Input.Blade.Eta)
            fprintf(f,formatD,  Input.Blade.Eta(i), Input.Blade.Chord(i),Input.Blade.Twist(i),...
    ...
    f=fopen(File,'w');
        ...
        fprintf(f,formatH,'% Radial Position [-]','Stiffness about Chord Line [Nm2]','Stiffness perpendicular to Chord Line [Nm2]','Torsional rigidity [Nm2]','Neutral axis [%chord]');
        
        fprintf(f,'\n');
        fprintf(f,'\n');
        fprintf(f,'\n');
        
        formatD='%-10.4f %-15.6e %-15.6e %-15.6e %-15.6f  \n' ;
        
        for i=1:length(Input.Blade.Eta)
            fprintf(f,formatD,  Input.Blade.Eta(i), Output.Blade.FlapBendingStiffnessRP(i),Output.Blade.EdgeBendingStiffnessRP(i),...
    ...
    f=fopen(File,'w');
        ...
        fprintf(f,formatH,'% Distance from root [-]','Centre of Mass [% chord]','Mass/unit length [kg/m]','Mom. Inertia/unit length [kgm]');
        
        fprintf(f,'\n');
        fprintf(f,'\n');
        fprintf(f,'\n');
        
        formatD='%-10.4f %-15.6f %-15.6f %-15.6f  \n' ;
        
        for i=1:length(Input.Blade.Eta)
            fprintf(f,formatD,  Input.Blade.Eta(i),       Output.Blade.MassCenterX_Perc(i),...
    ...
    fclose(f);
     ...
     % ==== Point Masses ==================================================
     % This file is no longer required by Cp-Lambda Pre-Processor version
     % 4.51 and later. I nevertheless leave this section here for possible
     % future use.. #LS
